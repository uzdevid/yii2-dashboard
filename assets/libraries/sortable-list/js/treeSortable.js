...
function TreeSortable(options) {
    ...
    this.defaults = {
        depth: 30, treeSelector: '#tree', branchSelector: '.tree-branch', branchPathSelector: '.branch-path', dragHandlerSelector: '.branch-drag-handler', placeholderName: 'sortable-placeholder', childrenBusSelector: '.children-bus', levelPrefix: 'branch-level', maxLevel: 10, dataAttributes: {
            id: 'id', parent: 'parent', level: 'level',
        }, removeBranch: true, buttons: {
            move: `<i class="bi bi-arrows-move"></i>`,
            addChildren: `<button type="button" class="button add-child" data-tippy-content="Add a new child"><i class="bi bi-plus"></i></button>`,
            addSibling: `<button type="button" class="button add-sibling" data-tippy-content="Add a new sibling"><i class="bi bi-plus-square"></i></button>`,
            removeBranch: `<button type="button" class="button remove-branch" data-tippy-content="Remove Branch"><i class="bi bi-trash"></i></button>`,
        }
    };
    ...
    this.createBranch = function ({id, parent_id, title, level, link, icon}) {
        const {
            branchSelector, branchPathSelector, dragHandlerSelector, childrenBusSelector, levelPrefix, dataAttributes: {id: idAttr, parent: parentAttr, level: levelAttr},
        } = self.options;

        return `<li class="${self.cleanSelector(branchSelector)} ${levelPrefix}-${level}" data-${idAttr}="${id}" data-${parentAttr}="${parent_id}" data-${levelAttr}="${level}">
            <div class="contents">
                <span class="${self.cleanSelector(branchPathSelector)}"></span>
                <div class="branch-wrapper">
                    <div class="left-sidebar">
                        <div class="${self.cleanSelector(dragHandlerSelector)}">
                            ${self.options.buttons.move}
                        </div>
                        <div>
                            <p class="branch-title m-0 p-0" data-value="${title}">${title}</p>
                            <p class="branch-link m-0 small text-muted" data-value="${link}">${link}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    this.jQuerySupplements = function () {
        ...
        $.fn.extend({
            getBranchLevel() {
                ...
                return this.each(function () {
                    let level = $(this).getBranchLevel() || 1, newLevel = level + dx;
                ...
                const $children = $(this).getChildren();
                const $descendants = $(this).getDescendants();
                const $lastChild = $descendants.length > $children.length ? $descendants.first() : $children.first();
                ...
                this.each(function () {
                    let level = $(this).getBranchLevel() || 1, $next = $(this).next(branchSelector);
                ...
                let level = $(this).getBranchLevel() || 1, $next = $(this).next(branchSelector), nextLevel = $next.getBranchLevel();
                ...
                const {branchSelector} = self.options;
                let level = $(this).getBranchLevel() || 1, $prev = $(this).prev(branchSelector), prevLevel = $prev.getBranchLevel();
                ...
                let $siblings = [], $branches = $(`${treeSelector} > ${branchSelector}`), $self = this;
    ...
    this.addChildBranch = function ($triggerElement, data) {
        const {treeSelector, branchSelector, dataAttributes: {id}, maxLevel} = self.options;
        ...
        const title = data ? data.title : 'New Branch';
        const link = data ? data.link : null;

        $firstChild = $branch.getFirstChild();

        const $element = self.createBranch({id: data.id, parent_id, title, level, link});
    ...
    this.addSiblingBranch = function ($triggerElement) {
        const {
            treeSelector, branchSelector, dataAttributes: {parent},
        } = self.options;
    ...
    this.updateBranchZIndex = function () {
        ...
        $branches.length && $branches.each(function (index) {
    ...
    this.initSorting = function () {
        const {
            treeSelector, dragHandlerSelector, placeholderName, childrenBusSelector, branchPathSelector, branchSelector, levelPrefix, dataAttributes, maxLevel,
        } = self.options;

        self.updateBranchZIndex();

        /** Store the current level, for sorting the item after stop dragging. */
        let currentLevel = 1, originalLevel = 1, childrenBus = null, helperHeight = 0, originalIndex = 0;
        ...
        const canSwapItems = ui => {
            let offset = ui.helper.offset(), height = offset.top + helperHeight, nextBranch = ui.placeholder.nextBranch(), nextBranchOffset = nextBranch.offset() || 0, nextBranchHeight = nextBranch.outerHeight();

            return height > nextBranchOffset.top + nextBranchHeight / 3;
        };

        $(treeSelector).sortable({
            handle: dragHandlerSelector, placeholder: placeholderName, items: '> *', start(_, ui) {
                ...
                let treeEdge = self.getTreeEdge(), offset = ui.helper.offset(), currentBranchEdge = offset.left, lowerBound = 1, upperBound = maxLevel;
                 ...
                 */
                let nextSibling = ui.placeholder.nextSibling(), placeholderLevel = 1;
    ...
    this.editBranch = function ($triggerElement, attribute, callback) {
        let $branch = $($triggerElement).closest('li.tree-branch');
        let $branchId = $branch.data('id');
        let $branchAttribute = $branch.find(attribute);

        let $branchInput = $(`<input name="title" value="${$branchAttribute.data('value')}">`);

        $($branchAttribute).html($branchInput);
        $branchInput.focus();

        return $branchInput.on('blur', function () {
            $branchAttribute.html($branchInput.val());
            callback($branchId, $branchInput.val());
        });
    }

    this.editIconBranch = function ($triggerElement, attribute, callback) {
        let $branch = $($triggerElement).closest('li.tree-branch');
        let $branchId = $branch.data('id');
        let $branchAttribute = $branch.find(attribute);
        ...
        return $branchInput.on('blur', function () {
            $branchAttribute.html('');
            $branchAttribute.addClass($branchInput.val());
            callback($branchId, $branchInput.val());
        });
    }
}

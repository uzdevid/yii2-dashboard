...
<?php foreach ($online_users as $user): ?>
    <li class="online-user-item position-relative">
        <img width="50px" src="<?php echo $user->profileImage; ?>" class="rounded-circle">
        <div class="ps-2">
            <h4><?php echo $user->fullname; ?></h4>
            <p>
                <i class="bi bi-circle-fill text-success m-0" style="font-size: 12px;"></i>
                <span><?php echo date('H:i', $user->last_activity_time); ?></span>
            </p>
        </div>
        <?php echo ModalPage::link('', Url::to(['/system/user/view', 'id' => $user->id]), ['class' => 'stretched-link']); ?>
    </li>
<?php endforeach; ?>
<? if (!empty($users)): ?>
    <hr>
